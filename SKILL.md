...
7. If the prompt uses a stored character or real-person-style reference, resolve the R2 asset first. Prefer the newest dated version under `r2:creative-assets/chars/<asset-slug>/`, presign it through the creative API storage layer, and pass the URL as `image_url`, `image_urls`, or `reference_images` according to the provider. In the generation prompt, refer to it only as `Reference [1]`, `Reference [2]`, etc.
